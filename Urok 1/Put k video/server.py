...
@app.route('/text_in_alert/<string:alert_text>')
...
def spisok(number):
    sp = ', '.join(map(lambda x: '{}: {}'.format(x, x), range(1, number+1)))
    return '''<!doctype html>
                <html lang="en">
                  <head>
                    <meta charset="utf-8">
...
def table(n, m):
    return render_template('table.html', n=n, m=m)

@app.route('/youtube/<int:number>')
def yputube(number):
    return render_template('youtube.html', n=number)
